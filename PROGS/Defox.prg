...
if m.lnErr=0 or between(m.lnErr, 1000, 9999)
	...
	_screen.Caption = 'Transport2007 тестовая v1.3 для СУ-257 Светлогорск'
	return
	...
	IF between(m.lnErr,20,39)
		m.lnOtv=messagebox("Невозможно прочитать информацию о программе из реестра."+chr(13)+ ;
			               ...
			               36, "Предупреждение")
		if m.lnOtv=7
			_screen.Caption = 'Transport2007 Неполноценная v1.3 '
			return
		endif
	...
	*---------------------------Пробуем еще раз проверить
	...
	DO CASE 
	   CASE m.lnErr=0 or between(m.lnErr, 1000, 9999)
	        _screen.Caption = 'Transport2007 тестовая v1.3 для СУ-257 Светлогорск'
	   OTHERWISE 
	        _screen.Caption = 'Transport2007 v1.3 DEMO-версия'
	ENDCASE 
